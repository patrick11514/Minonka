...
impl Task for GraphTask {
    ...
    async fn run(input: Self::Input, context: AppContext) -> TaskResult<TaskOutcome> {
        ...
        let pts: Vec<i32> = input.history.iter().map(|h| get_total_lp(&h.tier, &h.rank, h.lp)).collect();
        if pts.is_empty() {
        ...
        while current_div < y_max {
            ...
            let y_canvas_start = y_start - (((div_start - y_min) as f32 / (y_max - y_min) as f32) * plot_h as f32) as i32;
            let y_canvas_end = y_start - (((div_end - y_min) as f32 / (y_max - y_min) as f32) * plot_h as f32) as i32;
        ...
        draw_line_segment_mut(&mut canvas.background, (x_start as f32, y_end as f32), (x_start as f32, y_start as f32), border_color);
        draw_line_segment_mut(&mut canvas.background, (x_start as f32, y_start as f32), (x_end as f32, y_start as f32), border_color);
        draw_line_segment_mut(&mut canvas.background, (x_end as f32, y_end as f32), (x_end as f32, y_start as f32), border_color);
        draw_line_segment_mut(&mut canvas.background, (x_start as f32, y_end as f32), (x_end as f32, y_end as f32), border_color);
        ...
        for i in 0..(n_points - 1) {
            ...
            let y_i = y_start - (((pts[i] - y_min) as f32 / (y_max - y_min) as f32) * plot_h as f32) as i32;
            
            let x_next = x_start + ((i + 1) as f32 / (n_points - 1) as f32 * plot_w as f32) as i32;
            let y_next = y_start - (((pts[i + 1] - y_min) as f32 / (y_max - y_min) as f32) * plot_h as f32) as i32;
        ...
        for i in 0..n_points {
            ...
            draw_filled_circle_mut(&mut canvas.background, (x_i, y_i), 3, Rgba([255, 255, 255, 255]));
        }
